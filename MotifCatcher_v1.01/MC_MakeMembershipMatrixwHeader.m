function [MM MotifLengths]= MC_MakeMembershipMatrixwHeader(InputSet,SeqFileHeader,Headers,RevComp)
...
%Inputs:
%   InputsSet:
%       cell array of MEME output diredtories (whole path)
%   SeqFileHeader:
...
MotifLengths = zeros(1,length(InputSet));
    ...
    for i = 1:length(InputSet)
           ...
           for j = startlook:stoplook
              ...
              %last entry added for yeast analysis of intergenic reasons
                 ...
                 if RevComp == 1
                    ...
                    Sites{counter,2} = extraction{1,1}{j+1,1}; %Start 
                 end
              end
           end
           ...
           for k = length(Headers(:,1))+1:counter+length(Headers(:,1))
               for j = 1:length(SeqFileHeader)
                   if strfind(SeqFileHeader{1,j},Sites{k-length(Headers(:,1)),1})
                       
                       %record site
                       mats{1,i}{k,1} = SeqFileHeader{1,j};
                       
                       %record span of motif  
                       mats{1,i}{k,2} = [str2double(Sites{k-length(Headers(:,1)),2}),str2double(Sites{k-length(Headers(:,1)),2})+MotifLength-1];
...
% ----------------------------------------------------------------------- %
    ...
    for i = 2:length(mats)
        ...
        %and, a new header.
        for j = 1:length(Headers(:,1))
            MM{j,Motifs+1} = mats{1,i}{j,2};
        end
        
        for j = length(Headers(:,1))+1:length(mats{1,i});
            MakeNewRow = true;
            for k = length(Headers(:,1))+1:Rows
...
% ----------------------------------------------------------------------- %
    ...
    for i = length(Headers(:,1))+1:Rows-1
        for j = length(Headers(:,1))+1:Rows-1
